...
def test_dataset_data_prep_return_shape(nt_dataset_pinchoff, tmp_path):
    ds = Dataset(1, db_name="temp.db", db_folder=str(tmp_path))
    shape = tuple(nt.config["core"]["standard_shapes"]["1"])

    condensed_data = ds.prep_data("pinchoff")[0]
    ...
    with pytest.raises(KeyError):
        condensed_data = ds.prep_data("doubledot")[0]


def test_dataset_data_prep_normalization(nt_dataset_pinchoff, tmp_path):
    ds = Dataset(1, db_name="temp.db", db_folder=str(tmp_path))
    ...
    _ = ds.prep_data("pinchoff")[0]

    assert np.max(ds.data['dc_current'].values) <= 1
    assert np.min(ds.data['dc_current'].values) <= 0.5


def test_dataset_data_prep_return_data(nt_dataset_pinchoff, tmp_path):
    ds = Dataset(1, db_name="temp.db", db_folder=str(tmp_path))
    condensed_data = ds.prep_data("pinchoff")[0]
